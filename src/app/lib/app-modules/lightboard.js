...
export class LightboardModule extends AppModule {
    ...
    _getVolume() {
        if (this.api) {
            const lighboards = this.api.getAllDevices();
            if (lighboards[0]) {
    ...
    drawLights() {
        const bitmap = this.bitmap.slice(0);
    ...
    updateBitmap() {
        ...
        Object.keys(this.shapes).forEach((key) => {
            ...
            if (shape.type === 'rectangle') {
                for (var x = shape.x; x < shape.x + shape.width; x += 1) {
                    for (var y = shape.y; y < shape.y + shape.height; y += 1) {
                ...
                for (var x = -shape.radius; x <= shape.radius; x += 1) {
                    for (var y = -shape.radius; y <= shape.radius; y += 1) {
                ...
                shape.bitmap.forEach((color, index) => {
                    const coord = LightboardModule.indexToCoord(index, shape.width);
                    coord.x += shape.x;
                    coord.y += shape.y;
    ...
    _turnOn(light, color, cb) {
        if (light.type === 'all') {
            ...
            const index = LightboardModule.getIndex(light.x, light.y);
    ...
    _text(text, color, backgroundColor, cb) {
        this.lights = PixelFont.generateMatrix(text, color, backgroundColor);
        this.updateAndSync(false, cb);
    }
    ...
    _scrollNext() {
        const item = this.scrollStack.shift();
        if (!item) {
            this.scrolling = false;
            return;
        }
        ...
        const animationFrames = PixelFont.generateAnimation(item.text, item.color, item.backgroundColor);
